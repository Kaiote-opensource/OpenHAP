...
int MLX90640_DumpEE(MLX90640 *MLX90640_inst, uint16_t *eeData)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    return MLX90640_I2CRead(MLX90640_inst, 0x2400, 832, eeData);
}

int MLX90640_GetFrameData(MLX90640 *MLX90640_inst, uint16_t frameData[][834])
{
    /*Poll sensor to obtain subpage zero*/
...
//    uint8_t cnt = 0;
    ...
    error = MLX90640_I2CRead(MLX90640_inst, 0x0400, 832, frameData[1]); 
    if(error != 0)
    {
        return error;
    }
...
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    ESP_LOGD(TAG, "eeData[10] value obtained is [0x%04X]", eeData[10]);
...
int MLX90640_SetResolution(MLX90640 *MLX90640_inst, uint8_t resolution)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int value;
    ...
    if(error == 0)
    {
        ...
        error = MLX90640_I2CWrite(MLX90640_inst, 0x800D, value);        
    }    
...
int MLX90640_GetCurResolution(MLX90640 *MLX90640_inst)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int resolutionRAM;
...
int MLX90640_SetRefreshRate(MLX90640 *MLX90640_inst, uint8_t refreshRate)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int value;
...
int MLX90640_GetRefreshRate(MLX90640 *MLX90640_inst)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int refreshRate;
...
int MLX90640_SetInterleavedMode(MLX90640 *MLX90640_inst)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int value;
...
int MLX90640_SetChessMode(MLX90640 *MLX90640_inst)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int value;
...
int MLX90640_GetCurMode(MLX90640 *MLX90640_inst)
{
    ESP_LOGD(TAG, "ENTERED FUNCTION [%s]", __func__);
    uint16_t controlRegister1;
    int modeRAM;
...
//------------------------------------------------------------------------------
 ...
 int CheckEEPROMValid(uint16_t *eeData)  
 {
     ...
     ESP_LOGD(TAG, "Device select is [0x%04X]", deviceSelect);
     if(deviceSelect == 0)
     {
         return 0;
     }
